...
def generate_forced_parameters(original_parameters, genes):
    ...
    forced_parameters = get_forced_parameters(
        original_parameters, geneticized_parameters
    )

    forced_parameters = replace_forced_parameters(
        geneticized_parameters, forced_parameters
    )
...
def change_params_from_genes(genes):
    ...
    for param, optmize in plane_parameters_selector.items():
        if optmize == True:
            geneticized_parameters[param] = interp(
                genes[gene_index], genes_bounds, plane_parameters_bounds.get(param)
            )
            gene_index += 1
