...
class Airfoil:
    ...
    def generate_upper_surface_coordinates(self):
        ...
        self.n = self.airfoil_x_coords.size #número de elementos do vetor de coordenadas horizontais
        self.minpos = np.argmin(self.airfoil_x_coords) #posição do menor valor no vetor de coordenadas horizontais, determina a posição do bordo de ataque
        self.airfoil_x_coords_ext = np.flip(self.airfoil_x_coords[0:self.minpos]) #coordenadas horizontais do extradorso
    ...
    def generate_inner_surface_coordinates(self):
        ...
        self.airfoil_x_coords_int = self.airfoil_x_coords[self.minpos:self.n-1] #coordenadas horizontais do intradorso
        self.airfoil_y_coords_int = self.airfoil_y_coords[self.minpos:self.n-1] #coordenadas verticais do intradorso
        self.n_int = self.airfoil_x_coords_int.size #número de elementos do vetor de coordenadas horizontais do intradorso
    ...
    def calc_perimeter(self):
        ...
        for i in range (0,self.n_ext-2):
            delta_perimeter_i_ext = ((self.airfoil_x_coords_ext[i+1] - self.airfoil_x_coords_ext[i])**2 + (self.airfoil_y_coords_ext[i+1] - self.airfoil_y_coords_ext[i])**2)**0.5
        ...
        for i in range (0,self.n_int-2):
            delta_perimeter_i_int = ((self.airfoil_x_coords_int[i+1] - self.airfoil_x_coords_int[i])**2 + (self.airfoil_y_coords_int[i+1] - self.airfoil_y_coords_int[i])**2)**0.5
    ...
    def calc_area(self):
        ...
        for i in range (0,self.n_ext-2):
            delta_area_ext_i = ((self.airfoil_y_coords_ext[i] + self.airfoil_y_coords_ext[i+1]) * (self.airfoil_x_coords_ext[i+1] - self.airfoil_x_coords_ext[i]) * 0.5)
        ...
        for i in range (0,self.n_int-2):
            delta_area_int_i = ((self.airfoil_y_coords_int[i] + self.airfoil_y_coords_int[i+1]) * (self.airfoil_x_coords_int[i+1] - self.airfoil_x_coords_int[i]) * 0.5)
